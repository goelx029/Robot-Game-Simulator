...
Robot::Robot(const struct robot_params* const params) :
  ...
  motion_handler_.heading_angle(heading_angle_);
  motion_handler_.speed(2);
  motion_handler_.angle_delta(angle_delta_);
  motion_handler_.max_speed(max_speed_);
  motion_handler_.speed_delta(speed_delta_);
  id_ = next_id_++;
}
