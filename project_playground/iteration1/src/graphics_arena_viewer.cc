...
void GraphicsArenaViewer::DrawRobot(NVGcontext *ctx, const Robot* const robot) {
  ...
  // outer bar that stays same
  nvgBeginPath(ctx);
  nvgRect(ctx, robot->radius(), 0, 10, robot->maxcharge());
  ...
  // Making the inner bar that decrease
  double charge_ratio = 255*robot->chargelevel()/robot->maxcharge();
  nvgBeginPath(ctx);
  nvgRect(ctx, robot->radius(), 0, 9, robot->chargelevel());
