...
void GraphicsArenaViewer::DrawRobot(NVGcontext *ctx, const Robot* const robot) {
  ...
  nvgBeginPath(ctx);
  nvgRect(ctx, robot->get_radius(), 0, 10, robot->get_maxcharge());
  ...
  double charge_ratio = 255*robot->get_chargelevel()/robot->get_maxcharge();
  nvgBeginPath(ctx);
  nvgRect(ctx, robot->get_radius(), 0, 9, robot->get_chargelevel());
