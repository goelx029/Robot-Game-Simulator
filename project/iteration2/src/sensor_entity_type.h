...
class SensorEntityType : public Sensor {
 public:
  ...
  /**
   * @brief getters and Setters for different member variable.
   */
  const entity_type get_sensed_type() {return type_;}
  void set_sensed_type(entity_type e) {type_ = e;}

  const double get_range(void) { return range_; }
  void set_range(double r) { range_ = r; }

  const bool get_senttyp_activated(void) { return senttyp_activated_;}
