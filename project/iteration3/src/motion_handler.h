 ...
 public:
  ...
  virtual void Update_All(SensorTouch sensor_touch_,
    ...
    SensorDistress sensor_distress_) {
      (void)sensor_entity_type_;
      (void)sensor_touch_;
      (void)sensor_proximity_;
      (void)sensor_distress_;
    }

    /**
     * @brief function that updates the velocity and heading_angle of the
     * entity based on the sensor touch setting.
     */
  virtual void UpdateVelocity(SensorTouch st) {
    (void)st;
  }

  virtual bool get_freeze(void) {
    return false;
  }
