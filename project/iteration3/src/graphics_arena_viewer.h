...
class GraphicsArenaViewer : public GraphicsApp {
 public:
  ...
  /**
   * @brief Show the status of the Game (Win or Lost)
   *
   ...
   */
  void draw_status(NVGcontext *ctx);
 ...
 private:
  ...
  /**
   * @brief Draw the HomeBase in the arena using nanogui.
   *
   * This function requires an active nanovg drawing context (ctx), so it should
   * probably only be called from with \ref DrawUsingNanoVG().
   *
  ...
  /**
   * @brief Draw the (proximity)sensors of the robot in the arena using nanogui.
   *
   * This function requires an active nanovg drawing context (ctx), so it should
   * probably only be called from with \ref DrawUsingNanoVG().
   *
  ...
  /**
   * @brief Draw a superbot in the arena using nanogui.
   *
   * This function requires an active nanovg drawing context (ctx), so it should
   * probably only be called from with \ref DrawUsingNanoVG().
   *
   * @param[in] ctx The nanogui context.
   * @param[in] superbot The superbot handle.
   */
  void DrawSuperBot(NVGcontext *ctx, const class SuperBot* const superbot);

  /**
   * @brief Draw the (proximity)sensors of the superbot.
   *
   * This function requires an active nanovg drawing context (ctx), so it should
   * probably only be called from with \ref DrawUsingNanoVG().
   *
   ...
   * @param[in] superbot The superbot handle.
   */
  void DrawSuperBotSensors(NVGcontext *ctx,
    const class SuperBot* const superbot);
