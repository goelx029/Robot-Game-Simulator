...
void Arena::CheckForEntityOutOfBounds(const ArenaMobileEntity * const ent,
  ...
  if (ent->pos().x+ ent->radius() >= x_dim_) {
    ...
    event->angle_of_contact(ent->heading_angle()-180);  // edited this
  } else if (ent->pos().x- ent->radius() <= 0) {
    // Left Wall
    event->collided(true);
    event->point_of_contact(Position(0, ent->pos().y));
    event->angle_of_contact(ent->heading_angle());  // edited this
    ...
    event->angle_of_contact(270-ent->heading_angle());  // need to edit this
  } else {
