...
void RobotViewer::OnRestartBtnPressed()
{
  ...
  robot_land_->initialize();
}

void RobotViewer::OnColorChangeBtnPressed() { robot_land_->set_color(20,30,10); }
...
void RobotViewer::DrawRobot(NVGcontext *ctx, int id, double xpos, double ypos,
  ...
  nvgFillColor(ctx, nvgRGBA(robot_land_->get_rcol(id), robot_land_->get_gcol(id), robot_land_->get_bcol(id), 255));
  nvgFill(ctx);
